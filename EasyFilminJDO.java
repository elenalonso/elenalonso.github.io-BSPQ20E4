...
public class EasyFilminJDO implements IEasyFilminDAO{
	...
	public Director loadDirector(String name) {
		...
		try {
			...
			query.setFilter("Director == '" + name + "'"); //we find the director by his name
			query.setUnique(true);
			@SuppressWarnings("unchecked")
			...
			System.err.println(" $ Error retrieving dire ctors using a 'Query': " + ex.getMessage());
		} finally {
			if (tx != null && tx.isActive()) {
				tx.rollback();
			}
		...
		try {
		    ...
		    try {
				while ((values = readActors.readNext()) != null){
					...
					actors.add(new Actor(name,null,bday));
